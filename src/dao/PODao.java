...
public class PODao {
	...
	public ArrayList<PO> query(PO po) {
		sqlPO = "select *,"
				+ "DATE_FORMAT(createTime,'%Y-%m-%d %H:%i:%s') createTimeStr,"
				+ "DATE_FORMAT(updateTime,'%Y-%m-%d %H:%i:%s') updateTimeStr "
				+ "from " + tablePO + " where 1=1";
...
//		System.out.println("PO =" + po);
		...
		try {
			...
			while (rs.next()) {
				...
				rsPo.setCreateTime(rs.getString("createTimeStr"));
				rsPo.setUpdateUser(rs.getString("updateUser"));
				rsPo.setUpdateTime(rs.getString("updateTimeStr"));
				arr.add(rsPo);
			}
